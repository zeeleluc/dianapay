<div x-data="{ open: $wire.open }" class="relative language-switcher text-white text-sm w-full max-w-xs sm:max-w-none">
    <button
        type="button"
        @click="open = !open"
    ...
    <div
        ...
        class="dropdown-menu absolute right-0 mt-1 w-full sm:w-200 bg-darker border border-gray-700 rounded shadow-lg z-50 gap-2 p-2 text-sm"
    ...
    >
        @foreach ($allowedLocales as $lang)
            <button
                ...
                class="w-full flex items-center space-x-2 hover:bg-indigo-600 rounded px-2 py-1 text-left"
            >
